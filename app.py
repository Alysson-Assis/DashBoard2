...
def build_sidebar():
    st.image("Image/BuffaloCode.jpg")
    ticker_list = pd.read_csv("tickers_ibra.csv", index_col=0)
    tickers = st.multiselect(label="Selecione as Empresas", options=ticker_list, placeholder="Código")
...
def build_main(tickers, prices):
    ...
    prices['portfolio'] = prices.drop('IBOV', axis=1)  @ weigths
    ...
    with col2:
        ...
        fig.layout.yaxis.title = 'Risco Total'
...
st.title('BuffaloCode Invest.SA')
if tickers:
